...
import { FoodTruck, User, Town } from "@/api/entities";
...
export default function EditFoodTruck() {
  ...
  const loadTruck = async () => {
    try {
      ...
      const truckData = await FoodTruck.get(truckId);

      // Check if user owns this truck
      if (truckData.created_by !== userData.id) {
        alert("You don't have permission to edit this truck");
        navigate(createPageUrl("MyFoodTrucks"));
        return;
      }
      ...
      setFormData({
        ...
        photo_urls: truckData.photo_urls || [],
        cuisine_types: truckData.cuisine_types || [],
        dietary_flags: truckData.dietary_flags || [],
        payment_methods: truckData.payment_methods || [],
        preorder_url: truckData.preorder_url || "",
        website: truckData.website || "",
        phone: truckData.phone || "",
        social_links: truckData.social_links || {},
        town_id: truckData.town_id || "",
        base_town: truckData.base_town || "",
        typical_hours_note: truckData.typical_hours_note || "",
        menu_highlights: truckData.menu_highlights || [],
        accepts_preorders: truckData.accepts_preorders || false,
        family_friendly: truckData.family_friendly || false
      });

      // Load menu sections and operating hours
      setMenuSections(truckData.menu_sections || []);
      setOperatingHours(truckData.operating_hours || []);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await FoodTruck.update(truckId, updateData);
      navigate(createPageUrl(`FoodTruckDetail?id=${truckId}`));
    } catch (error) {
      console.error("Error updating food truck:", error);
      alert("Failed to update food truck. Please try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (!truck) {
    return <div className="min-h-screen bg-gradient-to-br from-orange-50 to-amber-50 flex items-center justify-center">
      <div className="text-center">
        ...
        <Button onClick={() => navigate(createPageUrl("MyFoodTrucks"))}>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-amber-50 p-6">
      <div className="max-w-4xl mx-auto">
        <Button
          variant="outline"
          onClick={() => navigate(createPageUrl(`FoodTruckDetail?id=${truckId}`))}
          className="mb-6"
        >
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-2">Update your food truck profile</p>
        </div>
