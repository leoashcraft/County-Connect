...
import { Product, Store, Brand, Town } from "@/api/entities";
...
export default function AddProduct() {
  ...
  const loadData = async () => {
    try {
      ...
      // Pre-fill contact info for one-off products
      if (!storeId) {
        setFormData(prev => ({
          ...prev,
      ...
      // Load brands and towns
      const [allBrands, allTowns] = await Promise.all([
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log("AddProduct: Submitting form");
    setSaving(true);
    try {
      console.log("AddProduct: Creating product", storeId ? `for store ${storeId}` : "as one-off listing");
      ...
      console.log("AddProduct: Product data:", productData);
      await Product.create(productData);
      console.log("AddProduct: Product created successfully");

      // Navigate based on whether it's a store product or one-off
      if (storeId) {
      ...
      console.error("AddProduct: Failed to create product:", error);
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-4 mb-8">
          <Button
            variant="outline"
            size="icon"
            onClick={() => navigate(store ? createPageUrl(`StoreManagement?id=${storeId}`) : createPageUrl("Marketplace"))}
          >
            <ArrowLeft className="w-4 h-4" />
          </Button>
          <div>
            <h1 className="text-3xl font-bold text-gray-900">
              {store ? "Add Product to Store" : "Post a Product"}
            </h1>
            <p className="text-gray-600 mt-1">
              {store ? `Add a new product to ${store.name}` : "List a product for sale in the marketplace"}
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {/* Seller Information (One-off products only) */}
          {!store && (
            <Card className="border-2 border-orange-200">
              <CardHeader>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="seller_phone">Contact Phone</Label>
                    <Input
                      id="seller_phone"
                      type="tel"
          ...
          <div className="flex justify-end gap-4">
            <Button 
              type="button" 
              variant="outline" 
              onClick={() => navigate(createPageUrl(`StoreManagement?id=${storeId}`))}
            >
              Cancel
            </Button>
            <Button
