    ...
    >
      ...
      <div className="max-h-64 overflow-y-auto">
        ...
        >
          ...
          {!currentCategory && <Check className="w-4 h-4 text-brand-600" />}
        </button>
        {categories.map((category) => (
          ...
          >
            ...
            {currentCategory === category.slug && <Check className="w-4 h-4 text-brand-600" />}
          </button>
        ))}
      </div>
    </div>,
...
export function ServicesFilters({ categories }: { categories: any[] }) {
  return (
